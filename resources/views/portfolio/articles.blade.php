...
@section('title', 'Portfolio Articles - Strategic Insights')
...
@section('content')
    ...
    <section class="relative bg-slate-900 pt-32 pb-24 overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-8 animate-fade-in">
            ...
            <p class="text-slate-400 text-lg max-w-2xl mx-auto">Exploring the intersection of technology, design, and business strategy.</p>
        </div>
    </section>
    ...
    <section class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="group bg-white rounded-[40px] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 flex flex-col">
                    ...
                    <div class="relative aspect-video overflow-hidden">
                        ...
                        <div class="absolute top-6 left-6">
                            <span class="px-4 py-2 bg-white/20 backdrop-blur-md rounded-full text-[10px] font-black text-white uppercase tracking-[0.2em] border border-white/20 shadow-xl">
                    ...
                    <div class="p-8 md:p-10 flex-1 flex flex-col space-y-6">
                        <div class="space-y-4">
                            <div class="flex items-center gap-3 text-[10px] font-black text-primary-600 uppercase tracking-widest">
                                <i data-lucide="calendar" class="w-3 h-3"></i>
                                {{ $article->created_at->format('M d, Y') }}
                            </div>
                            <h3 class="text-2xl font-display font-bold text-slate-900 group-hover:text-primary-600 transition-colors line-clamp-2 leading-tight">
                                {{ $article->title }}
                            </h3>
                        ...
                        <div class="pt-6 mt-auto">
                            <a href="{{ $article->type === 'link' ? $article->external_url : route('portfolio.articles.show', $article) }}" 
                               ...
                               class="inline-flex items-center gap-3 w-full justify-center px-8 py-4 bg-slate-900 text-white rounded-2xl font-bold text-sm hover:bg-primary-600 hover:scale-105 transition-all shadow-lg hover:shadow-primary-600/30">
                                ...
                                <i data-lucide="{{ $article->type === 'link' ? 'external-link' : 'arrow-right' }}" class="w-4 h-4"></i>
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-20 flex justify-center">
                <nav class="flex items-center gap-2">
                    <a href="#" class="w-12 h-12 rounded-xl bg-primary-600 text-white flex items-center justify-center font-bold">1</a>
                </nav>
            </div>
        </div>
    </section>
