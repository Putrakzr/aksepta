        ...
        <div class="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-2xl">
            <div class="bg-slate-900 p-12 text-white relative">
                ...
                <div class="relative z-10 flex items-center justify-between">
                    <div>
                        <h1 class="text-4xl font-display font-black tracking-tight">{{ isset($documentation) ? 'Edit' : 'Daftarkan' }} Dokumen.</h1>
                        <p class="text-slate-400 mt-2">Kelola aset repositori teknis dan strategis.</p>
                    </div>
                </div>
            </div>

            <form action="{{ isset($documentation) ? route('admin.documentations.update', $documentation) : route('admin.documentations.store') }}" method="POST" class="p-12 space-y-8">
                @csrf
