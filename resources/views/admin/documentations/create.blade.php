    ...
    <div class="min-h-screen bg-slate-50 p-6 pb-20">
        <div class="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-2xl">
                    ...
                    <div class="relative z-10 flex items-center justify-between">
                        <div>
                            <h1 class="text-4xl font-display font-black tracking-tight">
                                {{ isset($documentation) ? 'Edit' : 'Daftarkan' }} Dokumen.</h1>
                ...
                <form
                    action="{{ isset($documentation) ? route('admin.documentations.update', $documentation) : route('admin.documentations.store') }}"
                    method="POST" class="p-12 space-y-8">
                    @csrf
                    @isset($documentation) @method('PUT') @endisset
                    ...
                    <div class="grid md:grid-cols-2 gap-8">
                        <div class="space-y-3">
                            <label class="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Info Revisi
                                / Tanggal</label>
                        ...
                        <div class="space-y-3">
                            <label class="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">URL File /
                                Link</label>
                    ...
                    <div class="pt-6 border-t border-slate-100 flex justify-end">
                        <button type="submit"
                            class="inline-flex items-center gap-3 px-10 py-5 bg-slate-900 text-white rounded-3xl font-black transition-all hover:scale-110 hover:shadow-2xl hover:bg-amber-600">
                            <i data-lucide="file-check" class="w-5 h-5"></i>
                            {{ isset($documentation) ? 'Perbarui Dokumen' : 'Daftarkan Dokumen' }}
                        </button>
                    </div>
                </form>
            </div>
