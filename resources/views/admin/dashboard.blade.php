...
@section('content')
    <div class="h-screen bg-slate-50 flex overflow-hidden" x-data="{ sidebarOpen: window.innerWidth > 768 }">
        <!-- Mobile Backdrop -->
        <div x-show="sidebarOpen" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-40 md:hidden"
        ...
        <div class="bg-slate-900 text-slate-400 flex-shrink-0 transition-all duration-300 flex flex-col fixed inset-y-0 left-0 z-50 md:relative md:translate-x-0"
            ...
            <nav class="flex-1 p-4 space-y-8 overflow-y-auto">
                @foreach($navigation as $group => $items)
                    <div>
                        <h3 class="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-4"
                            x-show="sidebarOpen">
                            {{ $group }}
                        </h3>
                        <div class="space-y-1">
                            @foreach($items as $item)
                                <a href="{{ isset($item['route']) ? route($item['route']) : '#' }}" target="admin-content"
                                    ...
                                    <i data-lucide="{{ $item['icon'] }}"
                                        class="w-5 h-5 group-hover:text-primary-400 transition-colors"></i>
            ...
            <div class="p-4 border-t border-slate-800/50">
                <a href="/"
                    class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 transition-colors text-slate-500 hover:text-slate-300">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-8 shrink-0">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="text-sm font-bold text-slate-400 uppercase tracking-widest hidden sm:block">Control Center
                    </h2>
                </div>
            ...
            <div class="flex-1 relative bg-slate-50">
                ...
                <div id="frame-loader"
                    class="absolute inset-0 flex flex-col items-center justify-center z-10 bg-slate-50 transition-opacity duration-300 pointer-events-none">
                    <div class="w-12 h-12 border-4 border-primary-200 border-t-primary-600 rounded-full animate-spin mb-4">
                    </div>
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest animate-pulse">Memuat Data...</p>
                </div>

                <!-- The Magic Window -->
                <iframe name="admin-content" id="admin-content" src="{{ route('admin.projects.index') }}"
