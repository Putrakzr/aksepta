    ...
    <div class="bg-slate-900 rounded-3xl p-10 text-white relative overflow-hidden shadow-xl">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div class="space-y-3">
                ...
                <h1 class="text-4xl font-bold tracking-tight">Struktur <span class="text-emerald-400">Organisasi.</span></h1>
                <p class="text-slate-400 text-sm max-w-xl">Kelola tim dan pimpinan yang membangun ekosistem Aksepta.</p>
            </div>
            <a href="{{ route('admin.team.create') }}" class="inline-flex items-center gap-2 px-6 py-4 bg-white text-slate-900 rounded-2xl font-bold text-sm transition-all hover:scale-105 active:scale-95 shadow-lg">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Anggota</th>
                        <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Jabatan & Tipe</th>
                        <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Urutan</th>
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-sm">
                            <div class="flex flex-col">
                                <span class="text-slate-600 mb-1">{{ $member->position }}</span>
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $member->type }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2 translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
                                <a href="{{ route('admin.team.edit', $member) }}" class="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:text-emerald-600 hover:border-emerald-200 hover:shadow-md transition-all">
                                    <i data-lucide="edit-3" class="w-4 h-4"></i>
                                </a>
                                <form action="{{ route('admin.team.destroy', $member) }}" method="POST" onsubmit="return confirm('Hapus anggota tim ini?')">
                                    @csrf
                                    @method('DELETE')
