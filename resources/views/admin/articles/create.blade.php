        ...
        <div class="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-2xl">
            <div class="bg-slate-900 p-12 text-white relative">
                ...
                <div class="relative z-10 flex items-center justify-between">
                    <div>
                        <h1 class="text-4xl font-display font-black tracking-tight">{{ isset($article) ? 'Edit' : 'Draft' }} Artikel Baru.</h1>
                        <p class="text-slate-400 mt-2">Publikasikan wawasan strategis Anda berikutnya.</p>
                    </div>
                </div>
            </div>

            <form action="{{ isset($article) ? route('admin.articles.update', $article) : route('admin.articles.store') }}" method="POST" class="p-12 space-y-8">
                @csrf
                ...
                <!-- Conditional Fields -->
                <div id="manual_field" class="space-y-3 {{ old('type', $article->type ?? 'manual') == 'link' ? 'hidden' : '' }}">
                ...
                <div id="link_field" class="space-y-3 {{ old('type', $article->type ?? 'manual') == 'manual' ? 'hidden' : '' }}">
