            ...
            <div class="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-2xl">
                ...
                <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="p-12 space-y-8">
                    ...
                    <div class="grid md:grid-cols-3 gap-8">
                        <div class="space-y-3">
                            <label class="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Tipe
                                Artikel</label>
                            <select name="type" id="article_type"
                                ...
                                <option value="manual" {{ old('type', $article->type) == 'manual' ? 'selected' : '' }}>Konten
                                    Manual</option>
                                <option value="link" {{ old('type', $article->type) == 'link' ? 'selected' : '' }}>Link
                                    Eksternal</option>
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            @for ($i = 1; $i <= 3; $i++)
                                @php 
                                    ...
                                    $currentImage = $article->$fieldName;
                                @endphp
                                <div class="space-y-3">
                                    @if($currentImage)
                                        <div class="aspect-video bg-slate-50 rounded-2xl border border-slate-100 overflow-hidden mb-2">
                                            <img src="{{ asset($currentImage) }}" class="w-full h-full object-cover">
                                        </div>
                                    @endif
                                    <div class="relative">
                                        <label for="upload-{{ $i }}"
                                            class="flex flex-col items-center justify-center w-full h-32 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[32px] cursor-pointer hover:bg-slate-100 hover:border-emerald-300 transition-all group overflow-hidden"
                                            id="drop-zone-{{ $i }}">
                                            <div class="flex flex-col items-center justify-center py-2" id="placeholder-{{ $i }}">
                                                ...
                                                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Ganti Foto {{ $i }}</p>
                                            </div>
                    ...
                    <div id="manual_field" class="space-y-3 {{ old('type', $article->type) == 'link' ? 'hidden' : '' }}">
                        <label class="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Konten Artikel
                            (Mendukung Markdown/HTML)</label>
                    ...
                    <div id="link_field" class="space-y-3 {{ old('type', $article->type) == 'manual' ? 'hidden' : '' }}">
                        <label class="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">URL Artikel
                            Eksternal</label>
                    ...
                    <div class="pt-6 border-t border-slate-100 flex justify-end">
                        <button type="submit"
                            class="inline-flex items-center gap-3 px-10 py-5 bg-slate-900 text-white rounded-3xl font-black transition-all hover:scale-110 hover:shadow-2xl hover:bg-emerald-600">
